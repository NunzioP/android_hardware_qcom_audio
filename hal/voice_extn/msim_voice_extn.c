...
static msim_phone_type_t msim_phone_type = MSIM_SIM1;

int msim_voice_extn_start_call(struct audio_device *adev)
{
    audio_usecase_t usecase_id;

    usecase_id = msim_phone_type == MSIM_SIM2 ? USECASE_VOICE2_CALL : USECASE_VOICE_CALL;
                                             ...
                                             struct voice_session **session)
{
    ...
    idx = msim_phone_type == MSIM_SIM2 ? VOICE2_SESS_IDX : VOICE_SESS_IDX;
                                  ...
                                  struct str_parms *parms)
{
    ...
    if (ret >= 0) {
        ...
        if (!strcmp(value, AUDIO_PARAMETER_VALUE_CP2)) {
            msim_phone_type = voice_slot == 0 ? MSIM_SIM2 : MSIM_SIM1;
        } else {
            msim_phone_type = voice_slot == 0 ? MSIM_SIM1 : MSIM_SIM2;
        }
#elif SAMSUNG_DUAL_SIM
        msim_phone_type = property_get_int32(!strcmp(value, AUDIO_PARAMETER_VALUE_CP2) ?
...
int msim_voice_extn_get_active_session_id(struct audio_device *adev __unused,
                                         uint32_t *session_id)
{
    *session_id = msim_phone_type == MSIM_SIM2 ? VOICE2_VSID : VOICE_VSID;
    return 0;
}
